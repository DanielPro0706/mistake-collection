...
Size every fill-in underline to the expected answer itself rather than using a generic long rule. Match the visible line width to the typeset width of the complete answer, including its number, symbol, and unit; use an answer-measuring macro such as `\answerblankfor{...}` so the answer stays invisible while determining the width. For calculation, proof, experiment, geometry, circuit-design, and other large problems, reserve a modest amount of handwriting space in the question-only build. Base the space on the expected number and length of student steps, keep it compact enough for a mistake collection, and suppress that empty space in the answer build.
...
- **Writing usability:** confirm each large problem has a modest, usable writing area in the question-only PDF without excessive blank pages, and each fill-in underline matches the width of its expected complete answer rather than an arbitrary fixed length.
