...
18. Format every long solution as short, left-aligned lines. State constructions and point definitions in complete words such as `连接 AC、BD，交点记为 O`; do not encode them as `O=AC\cap BD`. Prefer inline formulas and do not center an isolated equality or short derivation. Do not use arrows such as `⇒`, `→`, `\Rightarrow`, or `\Longrightarrow` as substitutes for words or logical justification. Basic classroom inference may use `∵` and `∴`; in LaTeX, render them with math symbols such as `$\because$` and `$\therefore$` so glyphs are not lost.
...
For a calculation, experiment, proof, geometry, or circuit-design problem, show the verified standard answer's formal working when one is available; otherwise use independently derived classroom-standard working. Put each operation, condition, inference, or conclusion on its own left-aligned line. Use complete prose for constructions and nontrivial reasons. Use `∵` and `∴` only for basic causal steps, never implication arrows as proof shorthand. Keep formulas inline unless a genuinely long multi-line calculation requires alignment. Put independently authored commentary on the next left-aligned line in muted secondary red; do not use a two-column layout by default. Add a small red answer diagram when a direction, completed connection, auxiliary line, ray, force, state change, or instrument reading is clearer visually.
...
- **Large-problem standard steps:** for every large problem with a confirmed exact standard answer, compare the final formal work against its step order, notation, equations, intermediate conclusions, and scoring keywords. Confirm that steps are left-aligned and line-separated, constructions are written in words, short formulas remain inline, implication arrows are absent, and any independently authored commentary does not silently change the formal standard steps.
...
- **Composition:** keep the chapter title left-aligned, keep apparatus figures subordinate to the question text, and confirm long solutions use readable left-aligned lines without centered one-line formulas, dense proof paragraphs, default two-column notes, or implication-arrow shorthand.
