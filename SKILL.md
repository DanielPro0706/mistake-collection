...
description: Build a self-contained Chinese mistake collection from photographed or scanned school problems. Manually inspect originals, independently solve and web-cross-check answers, faithfully reconstruct figures, create synchronized question-only and black-question/red-solution PDFs, and write large solutions as left-aligned line-by-line classroom reasoning. Use for 错题整理、错题本制作、试题照片重排、题图重绘、双版本练习册、详细答案解析、初中规范解题步骤、or adding later problems to an existing collection.
---
...
8. If any question-bearing text, value, unit, option, label, or figure state remains blurred, cropped, obscured, or ambiguous after inspecting the original at full resolution, identify its exact location and ask the user for a clearer source before transcribing or finalizing the affected problem. Never guess it or borrow missing content from handwriting, OCR, web results, answer keys, or a similar problem.
...
18. Format every long solution as short, left-aligned formal steps paired with right-side explanations. Each row should contain one construction, condition, inference, calculation, or conclusion on the left and the explanation of that exact step on the right. State constructions and point definitions in complete words such as `连接 AC、BD，交点记为 O`; do not encode them as `O=AC\cap BD`. Prefer inline formulas and do not center an isolated equality or short derivation. Do not use arrows such as `⇒`, `→`, `\Rightarrow`, or `\Longrightarrow` as substitutes for words or logical justification. Basic classroom inference may use `∵` and `∴`; in LaTeX, render them with math symbols such as `$\because$` and `$\therefore$` so glyphs are not lost. When a single step is too wide for a readable two-column row, keep the formal step left aligned and place its commentary immediately below that step; return to paired columns for the following steps.
19. Treat every named or visibly printed segment as a separate connectivity invariant. A pair of labeled endpoints does not prove that a segment was drawn. For each panel, maintain a segment inventory that maps the source or wording (`连接 CE`, printed `AC`, extension `B-C-E`, and so on) to an actual continuous TikZ path or final-asset stroke. Check shared endpoints, collinearity, intersections, and extensions in the rendered PDF. Do not mark the figure exact when any required path is absent, disconnected, accidentally horizontal or vertical, or replaced by a nearby different segment.
20. Derive diagram geometry from constraints before choosing coordinates or proportions. Record collinearity, midpoint, parallel, perpendicular, equal-length, angle-bisector, and intersection conditions, then verify them algebraically or deterministically when using TikZ. Never place a point at a visually convenient height or on a convenient edge unless the source or conditions imply it. A visually plausible diagram fails when any recorded constraint is false.
...
If one intake batch contains multiple school subjects, split it into a separate project for each subject by default. Each subject project must have its own principal LaTeX source, question-only PDF, answer PDF, audit manifest, and required figure assets. Preserve the source order within each subject. Combine subjects only when the user explicitly requests one mixed collection.
...
- a panel-by-panel segment inventory listing every printed or instruction-required segment, ray, extension, and intersection, together with its endpoints and final drawing path;
...
Create a working `audit-manifest.json` from [assets/audit-manifest-template.json](assets/audit-manifest-template.json). New projects use schema version 2, which requires a segment inventory, a geometry-constraint list, and explicit verification flags for every figure; the validator remains backward-compatible with existing schema version 1 projects. Do not mark a problem `TEXT_EXACT` until its complete retained question block has been compared character by character with the full-resolution source. Do not mark an answer `ANSWER_CROSSCHECKED` until it has been independently solved and the required web search attempt has been recorded. Do not mark a figure `FIGURE_EXACT` until its source crop, final asset, both rendered PDFs, named segments, and geometry constraints have all been compared. The manifest is a verification artifact, not a substitute for manual inspection.
...
For a calculation, experiment, proof, geometry, or circuit-design problem, show the verified standard answer's formal working when one is available; otherwise use independently derived classroom-standard working. Put each operation, condition, inference, or conclusion on its own short left-aligned line in the left column. Put independently authored commentary for that exact line in the right column, aligned horizontally and rendered in muted secondary red. Use complete prose for constructions and nontrivial reasons. Use `∵` and `∴` only for basic causal steps, never implication arrows as proof shorthand. Keep formulas inline unless a genuinely long multi-line calculation requires alignment. If one row becomes cramped, place that row's commentary directly below its formal step rather than shrinking text or deleting explanation. Add a small red answer diagram when a direction, completed connection, auxiliary line, ray, force, state change, or instrument reading is clearer visually.
...
- **Named-segment connectivity:** reconcile the segment inventory against the final TikZ or asset and the rendered PDF. Require every named segment to be visibly continuous between the correct endpoints; verify compound collinear paths such as `B-C-E`, all shared vertices, and every requested connection. Labels or points without a stroke fail this gate.
- **Geometry constraints:** verify every recorded collinearity, midpoint, parallel, perpendicular, equal-length, angle-bisector, intersection, and ordering condition. For deterministic drawings, recompute coordinates or vector relations independently. Reject convenient but unjustified horizontal/vertical alignments and any point placed on the wrong line.
...
- **Large-problem standard steps:** for every large problem with a confirmed exact standard answer, compare the final formal work against its step order, notation, equations, intermediate conclusions, and scoring keywords. Confirm that formal steps are left-aligned and line-separated in the left column, each explanation is paired with the correct step in the right column (or immediately below only when that row is too wide), constructions are written in words, short formulas remain inline, implication arrows are absent, and commentary does not silently change the formal standard steps.
...
- **Composition:** keep the chapter title left-aligned, keep apparatus figures subordinate to the question text, and confirm long solutions use a readable workbook-style paired layout: formal work on the left, step-specific explanation on the right, with no centered one-line formulas, dense proof paragraphs, missing commentary, or implication-arrow shorthand. Allow a below-step explanation only for rows that cannot remain legible in two columns.
